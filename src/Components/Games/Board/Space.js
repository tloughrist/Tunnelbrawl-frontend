...
export default function Space({ color, id, setBoard }) { 

  const board = useContext(BoardContext);
  const [convertedBoard, setConvertedBoard] = useState(convert(board));
  const [occupant, setOccupant] = useState(findContents(parseInt(id)));
  
  function findContents(locNum) {
    const obj = convertedBoard.find(({loc}) => loc === locNum);
    return obj.contents;
  };
  ...
  function convert(board) {
    ...
    Object.keys(board).forEach((key) => {
      if (key.includes('loc')) {
        ...
        const spaceObj = {loc: parseInt(numLoc), contents: pieceConvert(board[key])};
